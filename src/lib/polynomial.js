...
function signClassOnReals(ce, coefficients) {
  ...
  if (oddAllZero) {
    ...
    if (!evenSigns.some((s) => s === null)) {
      ...
      if (!anyPositive && !anyNegative) return 'zero';
      if (!anyNegative) return constantTerm > 0 ? 'positive' : 'nonneg';
      if (!anyPositive) return constantTerm < 0 ? 'negative' : 'nonpos';
    }
  }
  ...
  if (degree === 2) {
    ...
    if (leading !== null && discriminant !== null && leading !== 0 && discriminant <= 0) {
      if (leading > 0) return discriminant < 0 ? 'positive' : 'nonneg';
      return discriminant < 0 ? 'negative' : 'nonpos';
    }
  }
...
function holdsOnDomain(ce, poly, variable, domain, kind) {
  switch (domain.kind) {
    ...
    case 'all': {
      ...
      if (coefficients && satisfiesSign(kind, signClassOnReals(ce, coefficients))) return true;
...
export function proveImplicationBySign(ce, left, right) {
  ...
  return holdsOnDomain(ce, right.diff, variable, domain, right.kind) ? true : null;
}
...
export function proveRelationBySign(ce, relation) {
  ...
  if (variables.length === 1) {
    ...
    if (coefficients) {
      if (decideRationalPolynomialRelation(coefficients, kind) === true) return true;
      if (satisfiesSign(kind, signClassOnReals(ce, coefficients))) return true;
    }
  }
